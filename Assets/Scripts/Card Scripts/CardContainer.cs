...
public class CardContainer : MonoBehaviour {
    ...
    {
        // Si la carta está en el área de juego, jugarla
        if (IsCursorInPlayArea())
        {
            // Invocar el evento de carta jugada
            eventsConfig?.OnCardPlayed?.Invoke(new CardPlayed(currentDraggedCard));

            // Llamar a OnCardUsed() del CardScript de la carta jugada
            var cardScript = currentDraggedCard.GetComponent<CardScript>();
            if (cardScript != null)
            {
                cardScript.OnCardUsed();  // Esta es la línea que debes agregar
            }

            // Destruir la carta si así se requiere
            if (cardPlayConfig.destroyOnPlay)
            {
                DestroyCard(currentDraggedCard);
            }
        }
        // Restablecer la carta arrastrada a null ya que el arrastre terminó
